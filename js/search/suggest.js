...
function suggest_init() {
  ...
  //requestAnimationFrame(silver_suggest_reposition);
  setTimeout(function () {
    silver_suggest.addListener('show', function () {
      silver_suggest_reposition();
    });
  }, 1);

  // prevent forced layout
  function silver_suggest_reposition() {
    if (silver_suggest_el.dataset.positioned) return;
    silver_suggest_el.dataset.positioned = true;
    var offsetLeft = q.offsetLeft, offsetTop  = q.offsetTop;
    var qStyle = window.getComputedStyle(q, null);
    var paddingLeft = parseInt(qStyle.getPropertyValue('padding-left'), 10);
    silver_suggest_el.style.top  = (offsetTop + 0) + "px"; //8
    silver_suggest_el.style.left = (offsetLeft + paddingLeft) + "px"; // 9+2
  ...
  */
  ...
  function keydown(e) {
      ...
      // return if there ares no suggestions (except ESC deletes input)
      if (!suggestions.innerHTML && e.keyCode != 27) return true;
      
      // collect link elements  
      var len, i;
      
      // resume if needed (except ESC deletes input)
      if (suggestions.style.display === 'none' && e.keyCode != 27) {
  ...
  if (byId("search-button"))
    byId("search-button").onclick = search;
  if (byId("search-button2"))
    byId("search-button2").onclick = search;
  ...
  function search(e) {
    ...
    if (fullUrlRegex.test(term) || nakedUrlRegex.test(term) || isValidIp(term)) {
      ...
      chrome.runtime.sendMessage({name: "search-event-url", url:term});
      suggestions.style.display = 'none';
      if (window.silver_suggest.suggestion) {
        byId("search-input").value = window.silver_suggest.suggestion;
        silver_suggest_el.style.display = 'none';
        chrome.runtime.sendMessage({name: "search-event-url-suggested", url:term});
      }
    ...
    chrome.runtime.sendMessage({"name": "search-event", term:term});
    suggestions.style.display = "none";
    remove_silver_suggest();

    if (settings.search_provider && settings.search_provider != 'home') {
      var searchProvider = searchProviders[settings.search_provider];
      var url = searchProvider.url.replace('${query}', encodeURIComponent(term));
  ...
  function initSearchIframe(param) {
    ...
    var searchTimeoutSlow3 = setTimeout(function () {
      chrome.runtime.sendMessage({"name": "search-timeout-3"});
    }, 3*SECONDS);
    var searchTimeoutSlow5 = setTimeout(function () {
      chrome.runtime.sendMessage({"name": "search-timeout-5"});
    ...
    window.on('message', function (e) {
      ...
      if ('DOMContentLoaded' == e.data) {
        ...
        change_options(function (settings_new) {
          settings_new.search_fullscreen = e.data.value;
        });
    ...
    if (settings.search_fullscreen) {
      ...
      wrap.style.cssText = "display:none; position: absolute; bottom: 0; top: 47px; width: 100%; z-index:0;";
  ...
  function updateSearchIframe(hash, term) {
    ...
    if (settings.search_fullscreen && 'string' == typeof hash) {
      ...
      byId("search-results").classList.add('fit');
  ...
  //  chrome.runtime.sendMessage({"name": "search-event"});
  //}
  ...
  /*
  // removed since include_js (main.js does this)
  window.on("click", function(e) {
  ...
  */
  ...
  silver_suggest.addListener('hide', function () {
    if (byId("search-input") == document.activeElement)
  ...
  function hide_search_bg_overlay() {
    ...
    bg_overlay && bg_overlay.remove();
  }


  window.suggest_keydown = keydown;
  window.suggest_term_changed = term_changed;
}
...
var activeId = search_provider;

var searchDropdownTimer;
byId('search-logo').onclick = function (e) { // onmouseenter
    //searchDropdownTimer = setTimeout(function () {
      include_js_once('js/search/search_provider.js', function () {
        searchProviderDropdownShow();
      });
    //}, 300);
}

byId('search-logo').onmouseleave = function (e) {
 clearTimeout(searchDropdownTimer);
}
...
// RHTab
//US|GB|CA|FR|NZ
//  &&  'true' == localStorage.rhtab_direct_test_2 
//if (US_geo) {
  onRHTabLoaded();
...
if (US_geo) {
  searchOverride = function (term) {
    ...
    ga_cb('search.send', 'event', 'search', 'search-infospace-direct', function () {
      window.location = "https://www.homenewtabsearch.com/i/" +
...
//  var feed = Feed.getInstance('gsp_hnt_00_00');
  searchOverride = function (term) {
    ...
    ga_cb('search.send', 'event', 'search', 'search-yahoo-direct', function () {
      //feed.search(term);
      // searchmake.com
      window.location = "https://searchdinam.com/?a=gsp_hnt_00_00&q=" + encodeURIComponent(term);
    });
    return true;
  };
}
